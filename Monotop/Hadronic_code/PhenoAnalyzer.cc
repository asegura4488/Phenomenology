  ...
  for(Int_t entry = 0; entry < numberOfEntries; ++entry)
    {
      ...
      double bjet_energy_1 = 0;
      double bjet_energy_2 = 0;

      double bjet_mass = 0;
      bool filled_first_jet = false;
      bool filled_second_jet = false;
      bool filled_first_bjet = false;
      bool filled_second_bjet = false;
	...
	{ 
	  ...
	  if((!is_jet_elec_overlap) && (!is_jet_muon_overlap)){
	    ...
	    if ((jet->PT > bjet_pt) && (TMath::Abs(jet->Eta) < bjet_eta) && (jet->BTag == 1)){
         if((filled_first_bjet == false) && (filled_second_bjet == false)){
	     bjet_energy_1 = calculateE(jet->Eta, jet->PT, jet->Mass);
	     bjet_i.SetPtEtaPhiE(jet->PT, jet->Eta, jet->Phi, bjet_energy_1);
	     Nbjets++;
	     filled_first_bjet == true; 
	     continue;
         }
         if((filled_first_bjet == true) && (filled_second_bjet == false)){
         bjet_energy_2 = calculateE(jet->Eta, jet->PT, jet->Mass);
         bjet_j.SetPtEtaPhiE(jet->PT, jet->Eta, jet->Phi, bjet_energy_2);
         Nbjets++;
         filled_second_bjet == true;
         continue;
         }
	    ...
	    if ((jet->TauTag == 0) && ((jet->BTag == 0))){
		  ...
		  if((filled_first_jet == false) && (filled_second_jet == false)){
		...
		if((filled_first_jet == true) && (filled_second_jet == false)){
	    ...
	    if ((filled_first_jet == true) && (filled_second_jet == true)){  
		double Mass_1 = (jet_1+jet_2).M();
        double Mass_2  = (jet_1+jet_2+bjet_i).M();
        if((Mass_1>0.)&&(Mass_1<2000.)){
        mass_w = Mass_1;
        mass_t = Mass_2;
        bjet_energy = bjet_i.E(); 
        energy_t = (jet_1+jet_2+bjet_i).E(); 
		hbratio = bjet_energy/energy_t;
	...
	pass_cuts[5] = 1;
	  ...
	  _hmap_N_bjet[i]->Fill(particle_counter[3]);
	   ...
	   if(delta_r > 0.)_hmap_delta_r[i]->Fill(delta_r); 
       if(pt_W > 0.)_hmap_pt_W[i]->Fill(pt_W);
       if(pt_top > 0.)_hmap_pt_top[i]->Fill(pt_top);
       if(mass_w > 0.){
	  ...
	  if(mass_t > 0.){
        ...
        if((bjet_energy > 0.) &&(energy_t > 0.))_hmap_Hbjet_ratio_pT[i]->Fill(hbratio);
	    if (energy_j1 > 0.){
      ...
      if(bjet_energy >0.)_hmap_bjet_energy[i]->Fill(bjet_energy);
	  if (elec_1.Pt() > 0.0) { 
...
double PhenoAnalysis::calculateE(double eta, double pt, double mass){
  ...
  double theta = TMath::ATan2(1,TMath::Exp(eta)); 
...
double PhenoAnalysis::CalculateP(double Eta1, double PT1){
  double theta1 = 2*TMath::ATan2(1,TMath::Exp(Eta1));
...
double PhenoAnalysis::CalculateP12_square_2particle(double Eta1, double PT1, double Eta2, double PT2){
  double theta1 = 2*TMath::ATan2(1,TMath::Exp(Eta1));
  double theta2 = 2*TMath::ATan2(1,TMath::Exp(Eta2));
  double pz1 = PT1*TMath::Cos(theta1)/TMath::Sin(theta1);
...
double PhenoAnalysis::CalculateP123_square_3particle(double Eta1, double Phi1, double PT1, double Eta2, double Phi2, double PT2, double Eta3, double Phi3, double PT3){
  double theta1 = 2*TMath::ATan2(1,TMath::Exp(Eta1));
  double theta2 = 2*TMath::ATan2(1,TMath::Exp(Eta2));
  double theta3 = 2*TMath::ATan2(1,TMath::Exp(Eta3));
  ...
  for (int i = 0; i < directories; i++)
    {
      ...
      _hmap_delta_r[i]          = new TH1F("delta_r", "delta_r", 100, 0. , 10.);
      _hmap_pt_W[i]             = new TH1F("pt_W", "pt_W", 200, 0. , 2000.);
      _hmap_pt_top[i]           = new TH1F("pt_top", "pt_top", 200, 0. , 2000.);
